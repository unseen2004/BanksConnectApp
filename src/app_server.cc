...
AppServer::HttpResponse AppServer::handleRequest(const HttpRequest& request) {
    ...
    if (request.method == "GET" && request.path == "/api/eb/transactions") {
        if (!apiAuthorized(request)) return unauthorized("Unauthorized");
        return jsonResponse(200, renderTransactionsJson());
    }
    if (request.method == "GET" && request.path == "/api/eb/balances") {
        if (!apiAuthorized(request)) return unauthorized("Unauthorized");
        return jsonResponse(200, renderAccountsJson());
    }
...

    {
            ...
            if (request.method == "GET") {
                ...
                for (size_t i = 0; i < txs.size(); ++i) {
                    ...
                    for (size_t j = 0; j < subs.size(); ++j) {
                        if (j) o << ",";
                        o << "{\"id\":"       << jsonString(subs[j].id)
                          << ",\"name\":"     << jsonString(subs[j].name)
                          << ",\"amount\":"   << subs[j].amount
                          << ",\"category\":" << jsonString(subs[j].category) << "}";
                    }
                    o << "]}";
                }
                o << "]";
        ...
        std::string tid = normPathParam("/api/transactions/");
        if (!tid.empty()) {
            if (!apiAuthorized(request)) return unauthorized("Unauthorized");
            std::string suffix = normPathSuffix("/api/transactions/", tid);
            if (suffix == "history" && request.method == "GET") {
                auto edits = db_->txHistory(tid);
                std::ostringstream o; o << "[";
                for (size_t i = 0; i < edits.size(); ++i) {
                    if (i) o << ",";
                    o << "{\"field\":" << jsonString(edits[i].field)
                      << ",\"old\":\"" << jsonEscape(edits[i].oldVal) << "\""
                      << ",\"new\":\"" << jsonEscape(edits[i].newVal) << "\""
                      << ",\"at\":"    << jsonString(edits[i].editedAt) << "}";
                }
                o << "]";
                return jsonResponse(200, o.str());
            }
            if (suffix == "split" && request.method == "POST") {
                ...
                while (true) {
                    pos = body.find("{", pos + 1);
                    if (pos == std::string::npos || pos < 10) break;
        ...
        if (normPath == "/api/budgets/summary") {
            if (!apiAuthorized(request)) return unauthorized("Unauthorized");
            std::string ym = query.count("month") ? query.at("month") : "";
        ...
        {
            std::string todoid = normPathParam("/api/todos/");
            if (!todoid.empty()) {
                if (!apiAuthorized(request)) return unauthorized("Unauthorized");
        ...
        if (normPath == "/api/savings") {
            ...
            if (request.method == "GET") {
                ...
                for (size_t i = 0; i < gs.size(); ++i) {
                    ...
                    o << "{\"id\":"       << gs[i].id
                      << ",\"name\":"     << jsonString(gs[i].name)
                      << ",\"target\":"   << gs[i].target
                      << ",\"deadline\":" << jsonString(gs[i].deadline)
                      << ",\"entries\":[";
